...
export class ExecutorHelper {
   ...
   */
  /*
  private addKillToCancellationToken(
    p: cp.ChildProcess,
    token?: vscode.CancellationToken
  ) {
    if (token) {
  ...
  */
  ...
  async handleDiagnosticErrors(
    ...
    const diagnosticMap: Map<
      string,
      Map<vscode.DiagnosticSeverity, vscode.Diagnostic[]>
    > = new Map();
    for (const error of errors) {
      if (error.line <= 0) {
        ...
        if (
          document &&
          (document.uri.toString() === canonicalFile || error.parentFile)
        ) {
          let text = "";
          // Processing path of included files
          if (error.parentFile) {
            const definitionHandler =
              ExtensionState.getCurrent().getDefinitionHandler();
            const includedFiles = await definitionHandler.getIncludedFiles(
              document.uri
            );
            ...
            const sourceDocument = await workspace.openTextDocument(
            ...
            if (sourceDocument) {
              ...
              if (sourceDocument.lineCount > sErrorLine) {
                const newRange = new vscode.Range(
                  ...
                  0,
                  sErrorLine,
                  sourceDocument.lineAt(sErrorLine).range.end.character + 1
                );
            ...
            if (document.lineCount > sErrorLine) {
              const newRange = new vscode.Range(
                ...
                0,
                sErrorLine,
                document.lineAt(sErrorLine).range.end.character + 1
              );
          ...
          if (pos > 0) {
            ...
            for (const fSymbol of sFile.getReferredSymbols()) {
              if (fSymbol.getLabel() == symbol) {
                const symbolErrorSeverity = this.mapSeverityToVSCodeSeverity(
                  error.severity
                );
                ...
                perErrorDiagnostics.push(
                  new vscode.Diagnostic(range, error.msg, symbolErrorSeverity)
                );
              }
            }
          }
    ...
    for (const [file, diagMap] of diagnosticMap) {
      ...
      const warningDiagnosticCollection =
        ExtensionState.getCurrent().getWarningDiagnosticCollection();
      const errorDiagnosticCollection =
        ExtensionState.getCurrent().getErrorDiagnosticCollection();
      if (
        ...
        if (newErrors && existingWarnings) {
          ...
          existingWarnings = existingWarnings.filter(
            (x) => errorLines.indexOf(x.range.start.line) === -1
          );
  ...
  getWorkspaceFolderPath(fileUri: Uri): string {
    if (fileUri) {
...
*/
  ...
  killTree(processId: number): void {
    if (process.platform === "win32") {
      ...
      try {
        winston.info(
          `Killing process with command ${TASK_KILL} /F /T /PID ${processId}'`
        );
