    ...
    if (thread) {
      try {
        ...
        if (stk.frames.length > 0) {
          ...
          const bpArray = this.breakpointManager.createTemporaryBreakpointArray(
            [frame.pc + 1, frame.pc + 2, frame.pc + 4]
          );
  ...
  protected async dataBreakpointInfoRequest(
    ...
    if (args.variablesReference !== undefined && args.name) {
      ...
      if (
        ...
        this.breakpointManager.populateDataBreakpointInfoResponseBody(
          response,
          variableName,
          displayValue,
          id.startsWith(WinUAEDebugSession.PREFIX_REGISTERS)
  ...
  protected async setDataBreakpointsRequest(
    ...
    if (args.breakpoints) {
      for (const reqBp of args.breakpoints) {
        ...
        const debugBp = this.breakpointManager.createDataBreakpoint(
          value,
          size,
          reqBp.accessType,
