...
export class UAEDebugSession extends LoggingDebugSession {
  ...
  protected async launchOrAttach(
    ...
    try {
      ...
      // Start the emulator
      if (startEmulator) {
        const emuArgs: string[] = [...(args.emulatorArgs ?? [])];
        if (!args.program) {
        ...
        const mountPath = args.program
          .replace(/\\/, "/")
          .replace(args.remoteProgram.replace(/^.+:/, ""), "");

        const binDir = findBinDir();
        let defaultBin: string;

        if (args.emulatorType === "fs-uae") {
          ...
          defaultBin = resolve(`${binDir}/fs-uae/fs-uae-${os}_x64`);

          // Set defaults to ensure we have the minimum CLI args to launch the emulator for debugging:
          if (debug) {
            if (!emuArgs.some((v) => v.startsWith("--remote_debugger="))) {
              emuArgs.push("--remote_debugger=10000");
            }
            if (!emuArgs.some((v) => v.startsWith("--remote_debugger_port"))) {
              emuArgs.push("--remote_debugger_port=" + args.serverPort);
            }
            if (
              !emuArgs.some((v) => v.startsWith("--remote_debugger_trigger"))
            ) {
              emuArgs.push("--remote_debugger_trigger=" + args.remoteProgram);
            }
          }
          if (
            !emuArgs.some(
              (v) => v.startsWith("--hard_drive") || v.match(/.adf/i)
            )
          ) {
            emuArgs.push("--hard_drive_0=" + mountPath);
          }
        } else {
          defaultBin = resolve(`${binDir}/winuae/winuae.exe`);
          if (debug) {
            if (!emuArgs.some((v) => v.startsWith("debugging_features"))) {
              emuArgs.push("-s", "debugging_features=gdbserver");
            }
            if (!emuArgs.some((v) => v.startsWith("debugging_trigger"))) {
              emuArgs.push("-s", "debugging_trigger=" + args.remoteProgram);
            }
          }
          if (
            !emuArgs.some((v) => v.startsWith("filesystem") || v.match(/.adf/i))
          ) {
            emuArgs.push("-s", "filesystem=rw,dh0:$" + mountPath);
          }
        }

        const executable = args.emulatorBin ?? defaultBin;

        logger.log(
          `[LAUNCH] Starting emulator: ${executable} ${emuArgs.join(" ")}`
        );
